...
本教材の全体像と各章の要点をまとめた学習ガイド（PDF）を用意しています：

📘 **[学習ガイド（PDF）](learning_guide.pdf)** - 関数一覧、数式、コード例を含む22ページの参考資料
...
- 6.3 カイ二乗検定
  - 適合度検定、独立性の検定
- 6.4 ベイズ分類器
- 6.5 クラスタリング（ckmeans: 1次元、kMeansCluster: 多次元）
- 6.6 時系列分析の基礎
  - 移動平均、指数移動平均、トレンド分析
- 6.7 実践例: A/Bテスト
- 6.8 練習問題
...
1. [JupyterLite](https://kklab.mobi/xeus-javascript/lab/) を起動
...
// simple-statistics の読み込み（ESM）
const ss = await import("https://esm.sh/simple-statistics@7.8.8");
```

> **注意**: URL中の `simple-statistics` に**空白が入ると読み込みエラー**になります。正確に記述してください。

## 対象読者

社会科学系の学生を対象としています。プログラミングと数学・統計学の両方のスキルを同時に習得することを目標としています。
...
- 学習ガイド（PDF）を追加
- 査読コメントに基づく改訂
  - t検定: 等分散仮定とt統計量返り値を明記
  - クラスタリング: ckmeans（1次元）とkMeansCluster（多次元）を区別
  - 仮説検定: p値/臨界値の両判定方式を記載
  - 母分散/標本分散の使い分けを明確化
  - 相関係数の解釈に注意事項を追加
